...
/****** Object:  StoredProcedure [dbo].[SP_Load_TC_P3_RHand]    Script Date: 9/10/2023 10:06:06 AM ******/
DROP PROCEDURE IF EXISTS [dbo].[SP_Load_TC_P3_RHand]
GO
...
CREATE PROCEDURE [dbo].[SP_Load_TC_P3_RHand]
(@P_Thread_id int)
AS
BEGIN
	...
	DROP TABLE IF EXISTS #tmp_TC_P3_RHand
	;

	CREATE TABLE #tmp_TC_P3_RHand(
	...
	BEGIN
	    INSERT INTO #tmp_TC_P3_RHand
		 ...
		 WHERE 1=1
		   ...
		   AND [HandID] NOT IN (SELECT [HandID] FROM [dbo].[TC_P3_RHand])
		   AND [HandID] NOT IN (SELECT [HandID] FROM #tmp_TC_P3_RHand)
		 ;
		 ...
		 BEGIN
			 INSERT INTO [dbo].[TC_P3_RHand]
			   ...
			   SELECT [HandID]
					   ...
					   ,[P2_R_RK]
					   ,[P3_R_RK]
				   FROM #tmp_TC_P3_RHand
				  WHERE 1=1
				    AND [HandID] NOT IN (SELECT [HandID] FROM [dbo].[TC_P3_RHand])
			 ;

			 TRUNCATE TABLE #tmp_TC_P3_RHand
			 ;
		 END
